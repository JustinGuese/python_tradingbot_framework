...
def test_darwinex_login(broker):
    with patch.object(broker.client, "post") as mock_post:
        mock_post.return_value = MagicMock(status_code=200, json=lambda: {"sessionToken": "test_token"})
        broker._login()
...
def test_darwinex_get_cash(broker):
    ...
    with patch.object(broker.client, "get") as mock_get:
        mock_get.return_value = MagicMock(status_code=200, json=lambda: {"balance": 10000.50, "equity": 12000.75})
...
def test_darwinex_place_order(broker):
    ...
    with patch.object(broker.client, "post") as mock_post:
        ...
        assert kwargs["json"] == {"instrument": "AAPL.US", "quantity": 10.0, "side": "BUY", "type": "MARKET"}
...
def test_darwinex_map_symbol(broker):
    ...
    with patch.object(broker.client, "get") as mock_get:
        # Mock search results
        mock_get.return_value = MagicMock(
            status_code=200, json=lambda: [{"symbol": "AAPL.US", "description": "Apple Inc", "type": "stock"}]
        )
...
def test_darwinex_cancel_orders(broker):
    ...
    with patch.object(broker.client, "get") as mock_get, patch.object(broker.client, "delete") as mock_delete:
        mock_get.return_value = MagicMock(
            status_code=200, json=lambda: [{"id": "o1", "status": "PENDING"}, {"id": "o2", "status": "PENDING"}]
        )
