...
@pytest.mark.parametrize(
    "yf_symbol, expected_broker, expected_type",
    [
        ("AAPL", "AAPL", "stock"),
...
def test_xau_has_no_default_rule():
    ...
    mapper = SymbolMapper(map_file="nonexistent.json")
    res = mapper.map_symbol("^XAU")
    assert res is not None
    assert res["symbol"] == "^XAU"
...
def test_hyperliquid_roundtrip():
    ...
    mapper = SymbolMapper()  # the real shipped symbol_map.json
    res_btc = mapper.map_symbol("BTC-USD", broker_name="hyperliquid")
    assert res_btc is not None
    assert res_btc["symbol"] == "BTC"
    assert mapper.unmap_symbol("BTC", broker_name="hyperliquid") == "BTC-USD"
    res_eth = mapper.map_symbol("ETH-USD", broker_name="hyperliquid")
    assert res_eth is not None
    assert res_eth["symbol"] == "ETH"
