...
class SwingTitaniumBot(Bot):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.last_trend: str | None = None
        self.active_trend_info: dict | None = None
        self.trend_switches: list = []
    ...
    def decisionFunction(self, row):
        ...
        troughs, peaks = (
            find_peaks(-sub_prices.values, distance=self.order, prominence=self.prominence)[0],
            find_peaks(sub_prices.values, distance=self.order, prominence=self.prominence)[0],
        )
        support_info = (
        ...
        trendline_price = self._line_value_at(active_trend_info, row["timestamp"]) if active_trend_info else np.nan
        price = row["close"]
        # Decision logic
        if active_trend == "uptrend" and self._is_touching_line(price, trendline_price):
            return 1  # Buy
        elif active_trend == "downtrend" and self._is_touching_line(price, trendline_price):
            return -1  # Sell/Short
        else:
            return 0  # Hold
    ...
    def _trendline_from_extrema(self, dates, prices, extrema_idx):
        if len(extrema_idx) < 2:
            return None
        x = np.array([dates.iloc[i].toordinal() + dates.iloc[i].hour / 24.0 for i in extrema_idx])
