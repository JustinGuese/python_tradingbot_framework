...
class SharpePortfolioOptWeeklyBot(Bot):
    ...
    def makeOneIteration(self):
        ...
        try:
            ...
            data_long = self.getYFDataMultiple(self.tradeable_symbols, interval="1d", period="3mo", saveToDB=True)
            ...
            logger.info(f"Top 5 holdings: {dict(sorted(weights.items(), key=lambda x: x[1], reverse=True)[:5])}")
