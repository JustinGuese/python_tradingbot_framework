...
class AIHedgeFundBot(Bot):
    ...
    def _get_latest_trading_decisions(self) -> dict | None:
        """
    ...
    def _convert_decisions_to_weights(self, trading_decisions: dict) -> dict:
        ...
        logger.info(
            f"Portfolio weights: {len(buy_symbols)} buy symbols (each {weight_per_buy:.2%}), "
    ...
    def makeOneIteration(self):
        ...
        try:
            ...
            logger.info(
                f"Short symbols (to sell): {[s for s, w in weights.items() if w == 0 and s in trading_decisions]}"
            )
