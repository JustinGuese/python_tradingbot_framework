    ...
    def makeOneIteration(self):
        ...
        if self.bot_name == "FearGreedBotQQQ":
            ...
            if self.currentFearGreed <= 25 and cash > 0:
                logger.info(f"Current FearGreed: {self.currentFearGreed} below {25} - Buying QQQ ")
                self.buy("QQQ")
                return 1
            elif self.currentFearGreed >= 75 and holding > 0:
                logger.info(f"Current FearGreed: {self.currentFearGreed} surpassed {75} - Selling QQQ ")
                self.sell("QQQ")
                return -1
            else:
                if holding > 0:
                    logger.info(f"Current FearGreed: {self.currentFearGreed} is neutral - Holding QQQ Inverse")
                    return 0
                elif cash > 0:
                    logger.info(f"Current FearGreed: {self.currentFearGreed} is neutral - Buying QQQ Inverse")
                    self.buy("QQQ")
                    return 1
                else:
                    logger.info(f"Current FearGreed: {self.currentFearGreed} is neutral - No action")
